function loopback_data = ZJY_256VSRC_WRITE(s,float_voltage_all_channels)
    ...
    reply = iNormalizeReply(loopback_data);
    if ~contains(upper(reply), "ACK, ALL CHANNEL UPDATE")
        error("opa_exp:ZJY_256VSRC_WRITE:AckMismatch", ...
            "Voltage source ACK mismatch: %s", char(reply));
    end
end

function text = iNormalizeReply(rawReply)
text = strtrim(replace(string(rawReply), char(0), ""));
end